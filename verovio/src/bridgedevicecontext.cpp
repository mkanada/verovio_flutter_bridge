    ...
    const std::string &text, const std::u32string &wtext, int x, int y, int width, int height)
{
    ...
    if (font->GetSmuflFont() != SMUFL_NONE) {
        ...
        // The alignment offset is not applied here: it depends on the total width of every
        // piece sharing this run's text-anchor chunk, which is not known until the chunk is
        // finalized (FinalizeTextChunk applies it uniformly to every run's bbox, and - for a
        // chunk with more than one piece - to `origin`/`alignment` too). x0 below is therefore
        // this run's own left edge, as if it were left-aligned at its raw pen position.
        const double x0 = run.origin.x;
        ...
        m_textChunkTextRuns.push_back(std::move(run));
...
{
    if (m_textChunkGlyphUses.empty() && m_textChunkTextRuns.empty()) {
        m_textChunkWidth = 0.0;
        return;
    }

    // A chunk made of exactly one common-text run and no glyph uses - by far the most common
    // case (a single-run title, tempo mark, dynamic...) - keeps letting the renderer do its own
    // alignment: score_bridge's SceneTextAlign.center/.right (scene_painter.dart) measures the
    // text with its own font shaping (TextPainter) and centers/right-aligns against
    // `run.origin`, which is more faithful than baking an offset from Verovio's own
    // GetTextExtent estimate. That self-measurement works only for a *single* run, though - it
    // has no way to know that several DrawText calls share one text-anchor. In the SVG, sibling
    // <tspan>s with no x of their own belong to the same text-anchor chunk (e.g. the
    // autogenerated page number, one <text text-anchor="middle"> wrapping
    // "<tspan>– </tspan><tspan class=\"num\">2</tspan><tspan> –</tspan>"); with more than one
    // contributing piece here (several text runs, and/or glyph uses mixed with text runs), each
    // renderer-side self-measurement would center/right-align only *its own* fragment instead of
    // the group, which is exactly the page-number mis-centering bug this branch fixes. From here
    // on, bake one shared offset - Verovio's own GetTextExtent, summed across every piece via
    // m_textChunkWidth, mirroring how resvg anchors a whole SVG text chunk - into `origin` too
    // and export the text runs left-aligned, so the renderer just draws each fragment where it
    // is told.
    const bool bakeTextRunPosition = !(m_textChunkGlyphUses.empty() && (m_textChunkTextRuns.size() == 1));
    ...
    for (BridgeTextRun &run : m_textChunkTextRuns) {
        // The bbox always reflects this run's real visual position (Verovio's own measurement),
        // whether or not `origin`/`alignment` end up baked below - it never gets a second,
        // renderer-side alignment pass the way `origin` does in the single-run case.
        if (run.hasBBox) {
            run.bbox[0] += offset;
            run.bbox[2] += offset;
        }
        if (bakeTextRunPosition) {
            run.origin.x += offset;
            run.alignment = HORIZONTALALIGNMENT_left;
        }
